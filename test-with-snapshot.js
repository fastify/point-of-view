...
test('reply.view with ejs engine, template folder specified, include files (ejs and html) used in template, includeViewExtension property as true, different mime type; requires TAP snapshots enabled', t => {
  t.plan(8)
  const fastify = Fastify()
  const ejs = require('ejs')
  const resolve = require('path').resolve
